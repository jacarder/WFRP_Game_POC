...
{
	public static BFSResult BFSGetRange(MapManager mapManager, Vector3Int startPoint, int movementPoints)
	{
		...
		while (nodesToVisitQueue.Count > 0)
		{
			...
			foreach (Vector3Int neighbourPosition in mapManager.FindAllTileNeighbors(currentNode))
			{
				TileScriptable tile = mapManager.GetTileAt(neighbourPosition);
				int nodeCost = tile.movementCost;
